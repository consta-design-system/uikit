...
import { ListExampleIndent } from './examples/ListExampleIndent/ListExampleIndent';
...
- [Содержимое](#содержимое-списка)
  ...
  - [Дополниельное наполнение](#дополниельное-наполнение)
...
- [Кастомизация](#кастомизация)
  ...
  - [Кастомные отсупы](#кастомные-отсупы)
...
### Дополниельное наполнение

В элемент списка можно добавить иконку и произволное наполнение.
...
Если в качестве значение в `leftSide` и `rightSide` указать массив
то отсупы между элементами проставится автомаически
...
Чтобы разбить элементы списка на группы, укажите признак присвоения группы к элемненту,
`groupId` в `items` или через свойство `getItemGroupKey`.
...
то отступы между передвнными элеменами расавятся автомаически.
...
За отступы элемента списка отвечает свойство `indent`. Если указать `increased`, то горизонтальный отступ будет увеличен. Значение по умолчанию `normal`.

Отступы зависят от свойсва `size`.
...
const ListExampleIndent = () => {
  return (
    <>
      <List items={items} indent="increased" />
      <List items={items} indent="normal" />
    </>
  );
};
```
...
Компонент позволяет поменять отсупы элементов `ListItem`, `ListGroupLabel`, `ListDivider`,
через `itemSpase`, `groupLabelSpase`, `dividerSpase` соответсвенно.

Свойства принимают объект как в [MixSpace](##LIBS.LIB.STAND/lib:uikit/stand:mixs-mixspace-stable).
Если необходимо убрать отсупы то передайте пустой объект `{}`
...
| [`indent?`](#отступы)                                   | `increased`, `normal`                                  | `normal`                     | Отступы компонента                                                     |
