...
describe.concurrent('getColumnLeftOffset', () => {
  ...
  test('получение отступа слева от колонки, если у колонки не изменялся размер', () => {
    const result = getColumnLeftOffset({
      columnIndex: 1,
      resizedColumnWidths: [],
  ...
  test('получение отступа слева от колонки, если у колонки изменялся размер', () => {
    const result = getColumnLeftOffset({
      columnIndex: 1,
  ...
  test('получение отступа слева от колонки, если не у всех колонок до выбранной изменялся размер', () => {
    const result = getColumnLeftOffset({
      columnIndex: 3,
...
describe.concurrent('getNewSorting', () => {
  ...
  test('устанавливает сортировку по полю, если было отсортировано по другому полю', () => {
  ...
  test('устанавливает сортировку по убыванию, если было отсортировано по возрастанию', () => {
    const result = getNewSorting<TestRow>(
      { by: 'field', order: 'asc' },
      'field',
    );
  ...
  test('убирает сортировку, если было отсортировано по убыванию', () => {
    const result = getNewSorting<TestRow>(
      { by: 'field', order: 'desc' },
      'field',
    );
...
describe.concurrent('useLazyLoadData', () => {
  ...
  test('отрезает часть массива видимых строк от исходного массива', () => {
  ...
  test('проставляет рефы необходимым ячейкам', () => {
