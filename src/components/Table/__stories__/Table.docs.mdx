...
Вы можете добавить какой-либо css-класс в зависимости от данных, которые содержатся в строке или колонке. Для этого надо задать таблице свойство `getAdditionalClassName`.

Данное свойство - функция, на вход которой подаётся объект `props` с тремя совйствами: `column`, `row` и `isActive`, где `column` - данные колонки, `row` - данные строки таблицы, а `isActive` - переменная, которая показывает, наведен ли курсор на текущую строку.
...
| [`getAdditionalClassName`](#добавление-класса-по-условию) | `(props: {column: any, row: any, isActive: boolean}) => string`      | -                                                 | Функция, которая на основе данных строки/колонки определяет css-класс, который нужно применить |
