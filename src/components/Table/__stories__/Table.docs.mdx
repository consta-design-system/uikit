...
`activeRow` выделяет строку, по которой осуществлен клик. [Подробней.](#обработка-клика-по-строке-и-ее-выделение)

`onRowHover` сработает при наведении на строку, полезно когда нужно скрыть/отобразить элементы в строке
...
Для того, чтобы выделить строку, передайте в компонент `Table` свойство `activeRow` типа `ActiveRow`
...
Обратите внимание, если вы передаете `id` активной строки, то при повторном клике по выделенной строке,
обработчик вам вернет `id = undefined`.
...
Если вы хотите только обрабатывать клики по строке, установите `id = undefined`
