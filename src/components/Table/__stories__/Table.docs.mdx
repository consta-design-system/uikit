...
- [Внешний вид](#внешний-вид)
  ...
  - [Добавление класса по условию](#добавление-класса-по-условию)
...
Вы можете добавить какой-либо css-класс в зависимости от данных, которые содержатся в строке или колонке. Для этого надо задать таблице свойство `getConditionalClassName`.

Данное свойство - функция, на вход которой подаются три аргумента: `column`, `row` и `isActive`, где `column` - данные колонки, `row` - данные строки таблицы, а `isActive` - переменная, которая показывает, наведен ли курсор на текущую строку.
Функция должна возвращать название css-класса типа `string`. При этом стиль должен быть добавлен с применением `!important`.
...
| [`getConditionalClassName`](#добавление-класса-по-условию) | `(column: any, row: any, isActive: boolean) => string`               | -                                                 | Функция, которая на основе данных строки/колонки определяет css-класс, который нужно применить |
