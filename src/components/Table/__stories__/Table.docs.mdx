...
`order` - порядок сортировки по умолчанию.
...
Стандартное поведение сортирует только строки.
Для того, чтобы предварительно отсортировать данные, добавьте свойство `order` типа `OrderType` в объект
с описанием колонки,
...
сортировка осуществится по первой колонке с этим свойством.

Через параметр `onSortBy` можно получить имя столбца и порядок сортировки,
