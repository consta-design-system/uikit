...
- [Внешний вид](#внешний-вид)
  ...
  - [Добавление стиля по условию](#добавление-стиля-по-условию)
...
### Добавление стиля по условию

Если вам нужно добавить какой-либо стиль в зависимости от данных, которые содержатся в строке или колонке, вы можете добавить его как css-класс. Для этого надо задать таблице свойство `getConditionalClassName`.
Данное свойство - функция, на вход которой подаются два аргумента: `column` и `row`, где `column` - данные колонки, `row` - данные строки таблицы.
Функция должна возвращать название css-класса типа `string`. При этом класс должен быть потомком класса `TableCell` и иметь свойство `&:not(&_isActive)`.
...
| [`getConditionalClassName`](#добавление-стиля-по-условию) | `(column: any, row: any) => string`                                  | -                                                 | Функция, которая на основе данных строки/колонки определяет css-класс, который нужно применить |
