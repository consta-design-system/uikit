...
#### Возможность сортировки для пользователя
...
С помощью этого свойства можно получить имя столбца и порядок сортировки, самостоятельно отсортировать данные и передать в `rows`. [Посмотреть примеры](#сортировка-по-времени-через-onsortby)
...
### Функция именования тегов фильтрации

Если вы хотите выводить не стандартный текст в теги фильтров или вам нужно привести данные к нужному формату(например дату вывести дату текстом), вы можете написать собственную функцию обработчик `getTagLabel`.

Он должен принимать в себя 3 аргумента

| № по счёту | Имя аргумента | Тип      | Описание                                               |
| ---------- | ------------- | -------- | ------------------------------------------------------ |
| 1          | `id`          | `string` | id фильтра из которого пришли два следующих аргумента  |
| 2          | `name`        | `string` | Имя, которое вы указывали в объекте фильтра            |
| 3          | `filterValue` | `any`    | Результат фильтрации, например, из кастомного элемента |
...
`activeRow` выделяет строку по клику [Подробнее](#обработка-клика-по-строке-и-ее-выделение)

`onRowHover` сработает при наведении на строку, это полезно, когда нужно скрыть или показать элементы в строке.

### Создать строки

Когда указана функция `onRowCreate`, в конце таблицы появляется кнопка создания строки.
Эта функция будет вызвана при нажатии на эту кнопку. В аргументах будет передан id последнего элемента или `undefined`(если таблица пуста).

Можно изменить текст "по умолчанию" на свой, если передать его в `rowCreateText`.

Если вам нужно добавить строки в другие места или удалять их, вы можете написать свой код с оглядкой на наш [ПРИМЕР](/?path=/story/components-table--with-row-actions).

Код примера можно найти в нашем [репозитории](https://github.com/gazprom-neft/consta-uikit/tree/dev/src/components/Table/__stories__/examples/WithRowCreationAndDeletion.tsx) GitHub.
...
| [`onRowCreate`](#создать-строки)                      | `(arg: { id?: string; index: number; e: React.MouseEvent }) => void` | -                                                 | Функция которая сработает при нажатии на кнопку создания строки |
| `rowCreateText`                                       | `string`                                                             | `'+ Добавить строку'`                             | Текст на кнопке создания новой строки                           |
| [`onRowHover`](#выделить-строки)                      | `(arg: { id: string | undefined; e: React.MouseEvent }) => void`     | -                                                 | Функция которая сработает при наведении на строку               |
| [`onRowClick`](#выделить-строки)                      | `(arg: { id: string; e: React.MouseEvent }) => void`                 | -                                                 | Функция которая сработает при клике на строку                   |
...
| [`getTagLabel`](#функция-именования-тегов-фильтрации) | `(id: string, name: string, filterValue:any) => string`              | getTagLabelDefault                                | Функция создания имени в тегах фильтров                         |
...
обработчик вернет `id === undefined`.
