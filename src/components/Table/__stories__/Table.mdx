...
`externalFiltration` - происходит ли фильтрация вне таблицы. Если равен `true`, то фильтр внутри таблицы применён не будет
...
type CustomFilters<T extends TableRow> = { [key in RowField<T>]?: CustomFilter }
...
| `numberOfRows`                                      | `numberOfRows`                                      | rows.length                                                                     | Общее количество строк. Используется при передаче в таблицу массива строк, отфильтрованных на сервере. Если не задано, то равняется длинне массива rows |
| `getResultText`                                     | `(count: number, total: number) => string`          | `` (count: number, total: number) => `Найдено ${count} из ${total} значений` `` | Функция для генерации текста с результатами фильтрации                                                                                                  |
| ;`                                                  |
