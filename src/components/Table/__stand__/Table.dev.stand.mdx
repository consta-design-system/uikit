...
- [Фильтры](#фильтры)
  ...
  - [Коллбэк на применение фильтра](#коллбэк-на-применение-фильтра)
...
- сначала [строки](#строки) в свойстве `rows`,
- затем [колонки](#колонки) в свойстве `columns`.

Ещё можно добавить [фильтры](#фильтры) в свойстве `filters`, настроить [сортировку](#сортировка) и [внешний вид](#внешний-вид).
...
Все объекты строк должны наследовать тип `TableRow`:
...
`getComparisonValue` — коллбэк для сравнения ячеек. Принимает значение ячейки, возвращает строку для сравнения.
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-зафиксированным-заголовком)
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-зафиксированной-колонкой)
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-со-скрытыми-колонками)
...
Если в ваших ячейках **нестандартные данные** или вам нужно как-то иначе их сравнивать, чтобы объединить (например, сравнить строки с разным регистром), вы можете использовать коллбэк `getComparisonValue`.
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-ячейками--объединёнными-кастомной-функцией)
...
Чтобы создать кастомные ячейки в колонке, добавьте в объект с описанием этой колонки функцию `renderCell`.
...
Чтобы функция работала, нужно ограничить контент по ширине. Для этого в параметрах `columns` укажите ширину ячейки (по умолчанию ширина не ограничена и контент не обрезается, а рендериться целиком).
...
За изначальное разворачивание строки отвечает поле `defaultExpand` в объекте строки:
...
Или можно в свойствах таблицы указать свойство `defaultExpandAll`, которое при значении `true`, развернет все вложенные строки
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-дополнительным-элементом-в-заголовке)
...
[Пример в работе](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-добавлением-удалением-строк)
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-со-своим-текстом--если-нет-данных)
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-большим-количеством-строк)
...
- `e: React.SyntheticEvent` — событие клика
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-обработкой-клика-по-ячейке)
...
Таблица отсортируется по первой колонке с этим свойством — по алфавиту (`'asc'`, `'ASC'`) или в
...
export function TableExampleFilter() {
  return <Table columns={columns} rows={rows} filters={filters} />;
}
```
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-кастомными-фильтрами)
...
| `TableChoiceGroupFilter` | `{ items: {name: string, value: any} }`                               | Фильтрация по одному из значений   |
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-со-своей-функцией-переименования-тега-в-фильтре)
...
Чтобы обработать наведение на строку, используйте `onRowHover`. Например, если нужно скрыть или показать элементы в строке.
...
[Посмотреть пример](##LIBS.LIB.STAND/lib:uikit/stand:components-table-deprecated/tab:use/hash:пример-таблицы-с-дополнительным-классом)
...
| [`onFiltersUpdated?`](#коллбэк-на-применение-фильтра)            | `(filters: { field: string[] }) => void`                                                                                     | -                                                 | Коллбэк на изменение фильтров                                                                                         |
