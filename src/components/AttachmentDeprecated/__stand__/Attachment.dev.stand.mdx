...
```tsx
import { Attachment } from '@consta/uikit/AttachmentDeprecated';
```
...
Чтобы показать, что файл загружается, используйте свойство `loading`. Тогда вместо иконки файла пользователь увидит индикатор загрузки.
...
Если во время загрузки файла произошла ошибка, вы можете показать, что случилось, с помощью свойства `errorText`.

Постарайтесь в тексте ошибки объяснить пользователю не только, что произошло, но ещё и почему так получилось и как это исправить.
...
`buttonIcon` отвечает за иконку на кнопке, она появится при наведении на компонент. Иконку можно выбрать из [библиотеки иконок](##LIBS.LIB/lib:icons).
...
| [`loading`](#загрузка-файла)              | `boolean`                               | -            | Индикатор загрузки                                               |
