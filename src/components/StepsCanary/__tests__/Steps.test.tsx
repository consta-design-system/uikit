...
import { cnStepsItem } from '../StepsItem/StepsItem';
...
const getItems = () => getRender().querySelectorAll(`.${cnStepsItem()}`);

const getButton = (index = 0) =>
  getItems()[index].querySelector(`.${cnStepsItem('Button')}`) as Element;
  ...
  describe('проверка props', () => {
    ...
    describe('проверка getLabel', () => {
      it('лейбл совпадает', () => {
        renderComponent({});
        expect(
          getItems()[0].querySelector(`.${cnStepsItem('Label')}`)?.textContent,
