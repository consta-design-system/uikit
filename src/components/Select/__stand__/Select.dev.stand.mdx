...
> Для создания hover эффекта для view="`clear`" управляйте внешним видом контейнера, в котором находится Select
...
| `onScrollToBottom?`                                                     | `(length: number) => void`                             | -                         | Коллбэк сработает при прокрутки до конца списка                           |
