...
```tsx
import { RadioGroup } from '@consta/uikit/RadioGroupCanary';
```
...
Одну или несколько радиокнопок можно сделать неактивными — если вы не хотите, чтобы их выбирали. За неактивность отвечает функция `getItemDisabled` или указав свойство `disabled` в элементе списка.
...
| [`getItemLabel?`](#название-радиокнопки)      | `(item: ITEM) => string`  | -            | Функция для формирования текста для радиокнопки. Также используется в качестве уникального ключа элемента |
| [`getItemDisabled?`](#неактивная-радиокнопка) | `(item: ITEM) => boolean` | -            | Функция для определения неактивного состояния радиокнопки                                                 |
