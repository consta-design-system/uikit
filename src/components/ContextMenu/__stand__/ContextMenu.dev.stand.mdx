...
```tsx
<>
  const items = ['Пункт 1', 'Пункт 2', 'Пункт 3']; const [isOpen, setIsOpen] =
  useState(false); const ref = useRef(null); return (
  <>
...
```tsx
<>
  const [isOpen, setIsOpen] = useFlag(); const buttonRef = useRef(null); return
  (
  <>
