...
describe('compare', () => {
  ...
  describe('equalHex', () => {
    it('возвращает true для одинаковых hex в одном регистре', () => {
      expect(equalHex('#ff0000', '#ff0000')).toBe(true);
      expect(equalHex('ff0000', 'ff0000')).toBe(true);
    });

    it('возвращает true для hex в разном регистре', () => {
      expect(equalHex('#FF0000', '#ff0000')).toBe(true);
      expect(equalHex('FF0000', 'ff0000')).toBe(true);
    });

    it('возвращает false для разных hex', () => {
      expect(equalHex('#ff0000', '#00ff00')).toBe(false);
    });

    it('сравнивает сокращённые формы', () => {
