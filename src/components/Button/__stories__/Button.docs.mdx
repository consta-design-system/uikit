...
Размер иконки равен размеру кнопки.

Чтобы поменять размер иконки на кнопке, используйте свойство `iconSize`, в котором можно указать любой размер из списка допустимых: `m`, `s`, `xs`.
