...
import {
  ExampleNameSemantics,
  ExampleNameVerb,
  ExampleNameNavigation,
  ExampleNameSingleLine,
  ExampleNameAgreement,
  ExampleNameOverflow,
} from './examples/ExampleName/ExampleName';
import {
  ExampleViewPrimary,
  ExampleViewSecondary,
  ExampleViewGhost,
  ExampleViewClear,
  ExampleViewDisabled,
} from './examples/ExampleView/ExampleView';
import { ExampleFormBasic, ExampleFormHybrid } from './examples/ExampleForm/ExampleForm';
import { ExampleSizeBasic, ExampleSizeFull } from './examples/ExampleSize/ExampleSize';
import { ExampleIconBasic, ExampleIconOnly } from './examples/ExampleIcon/ExampleIcon';
...
Название кнопки можно дополнять иконкой. Она поможет быстрее найти нужную кнопку в ряду других и быстрее понять, что эта кнопка делает. За иконку отвечают свойства `iconLeft` (ставит иконку слева от текста на кнопке) или `iconRight` (ставит иконку справа). Оба эти свойства ожидают компонент иконки из нашей [библиотеки иконок](?path=/docsx/icons--icons).
...
Для комбинации кнопки с [полем ввода](?path=/story/textfield--%D1%82%D0%B5%D0%BA%D1%81%D1%82%D0%BE%D0%B2%D0%BE%D0%B5-%D0%BF%D0%BE%D0%BB%D0%B5-%D0%B2%D0%B2%D0%BE%D0%B4%D0%B0) предусмотрены гибридные формы. У таких вариантов правая и левая грани могут отличаться.
