...
Если указать значение `input`, слева или справа появится поле ввода со значением, выбранным на слайдере.
Если поменять значение в поле, ползунок на слайдере подвинется (в тот момент, когда поле потеряет фокус).
...
Используйте поля ввода с обеих сторон только с применением модификации `range`
