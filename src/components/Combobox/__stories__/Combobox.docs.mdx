...
import { ComboboxExampleOnSearch } from './examples/ComboboxExampleOnSearch/ComboboxExampleOnSearch';
...
Для того чтобы получить значение поисквого поля необходимо указать колбек для `onSearch`.
...
| [`onSearch?`](#поиск)                                     | `(args: e: React.ChangeEvent; id?: string, number; name?: string; value: string, null;) => void;` | -                                                             | Функция для получения значения поля ввода                              |
...
| `labelForNotFound?`                                       | `string`                                                                                          | `Не найдено`                                                  | Текст, который нужно показать, если при поиске ничего не нашлось       |
| `labelForCreate?`                                         | `string`                                                                                          | `+`                                                           | Текст для кнопки, по которой можно добавить элемент в список           |
