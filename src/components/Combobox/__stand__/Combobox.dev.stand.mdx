...
Чтобы получить значение поисквого поля, нужно указать колбек для `onSearchValueChange`.
...
const useMockLoadData = (
  ...
  const items = useMemo(
    () =>
      new Array(lenght).fill(null).map((_, i) => ({
        label: `Опция ${i + 1}`,
        id: i,
      })),
    [lenght],
...
| `onScrollToBottom?`                                                     | `(lenght: number) => void`                             | -                                                             | Событие срабатывает когда доскролили список до конца                                  |
