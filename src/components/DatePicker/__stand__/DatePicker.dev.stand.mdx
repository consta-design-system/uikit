...
| `locale?`                                                                                                      | `Locale`                                                                                                                                                                                  | `ru`                  | Локализация календаря                                                      |
...
Если заданы оба варианта, `timeOptions` имеет приоритет над `multiplicity*`.
Если `timeOptions` не указан для конкретного параметра (часы/минуты/секунды), будут использоваться значения из `multiplicity*` (при их наличии).
...
**Важно:**
`timeOptions` применяется только для ограничения значений в выпадающем списке.
В поле ввода действуют только базовые ограничения на время (часы 0-23, минуты/секунды 0-59).
Однако при вводе времени, которое не соответствует timeOptions, возвращается ошибка типа `'invalidTimeByTimeOptions'`.
Подробнее об обработке ошибок см. в разделе [Обработка ошибок](#обработка-ошибок).
...
- `invalidTimeByTimeOptions` — введённое время не соответствует ограничениям `timeOptions`.
