...
type DatePickerTypeYearRangeProps = React.ComponentProps<
  typeof DatePicker<'year-range'>
>;

const renderComponent = (
  ctx: TestContext,
  props: DatePickerTypeYearRangeProps = {},
) => {
...
describe.concurrent('Компонент DatePicker_type_yearRange', () => {
  ...
  describe.concurrent('проверка value', () => {
    ...
    test(`верно отображается в календаре`, (ctx) =>
      context.start(async () => {
        ...
        expect(getDateTimeItemSelected(ctx, 0)).toHaveTextContent('1970');
        expect(getDateTimeItemSelected(ctx, 1)).toHaveTextContent('1973');
      }));
  });
});
