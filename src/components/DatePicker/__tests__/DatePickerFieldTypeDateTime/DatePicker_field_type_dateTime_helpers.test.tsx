    ...
    priorityCases.forEach(
      ({
        ...
        it(label, () => {
          ...
          (['HH', 'mm', 'ss'] as const).forEach((key) => {
            const block = config.blocks[key];
            expect(block.mask.getMockName()).toBe(expectedMasks[key]);
            if (expectedMasks[key] === 'MaskedEnum') {
              expect(Array.isArray(block.enum)).toBe(true);
              const values: string[] = block.enum;
              if (expectedLengths && expectedLengths[key]) {
          ...
          expect(config.blocks.yyyy.mask.getMockName()).toBe('MaskedRange');
          expect(config.blocks.MM.mask.getMockName()).toBe('MaskedRange');
          expect(config.blocks.dd.mask.getMockName()).toBe('MaskedRange');
        });
      },
    );
  });
