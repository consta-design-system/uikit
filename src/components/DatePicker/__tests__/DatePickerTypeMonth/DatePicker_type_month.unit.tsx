...
type DatePickerTypeMonthProps = React.ComponentProps<
  typeof DatePicker<'month'>
>;

const renderComponent = (
  ctx: TestContext,
  props: DatePickerTypeMonthProps = {},
) => {
...
describe.concurrent('Компонент DatePicker_type_month', () => {
  ...
  describe.concurrent('проверка value', () => {
    ...
    test(`верно отображается в календаре`, (ctx) =>
      context.start(async () => {
        ...
        expect(getDateTimeItemSelected(ctx)).toHaveTextContent('янв');
      }));
  });
});
