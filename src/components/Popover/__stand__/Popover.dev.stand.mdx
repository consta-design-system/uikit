...
По умолчанию вьюпортом является `document.documentElement`.
При неоходимости можно его заменить на любой другой элемент, используйте свойство `viewportRef`.
Обычно это требуется когда вьюпорт имеет скролл и необходимо не показвать `Popover` за пределами вьюпорта.
...
| [`viewportRef?`](#viewport)                          | `React.RefObject<HTMLElement>`                                       | -                       | Ссылка на вьюпор                                                                                            |
