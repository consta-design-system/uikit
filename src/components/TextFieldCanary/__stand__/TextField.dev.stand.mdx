...
- [Тип поля](#тип-поля)
  ...
  - [type="textarray"](#поле-для-ввода-массива-строк--type--textarray-)
...
Можно имортировать один общий компонет
...
## Поле для ввода массива строк: `type="textarray"`
...
Вы можете поменять рендер каждого элемента значения поля, используйте свойсво `renderValueItem`.
...
Иногда при вставке строки необходимо ее сразу разьивать на подстроки о добавлять к `value`.

Пример:
Есть список email'ов который при вставке необходимо преобразавать в массив строк и задать `value`.
...
| [`incrementButtons?`](#шаг-набор-значений-и-диапазон) | `boolean`                                                                                                                                                                                                                                                                                                                                                                                                       | `false`                               | Отображать ли кнопки-стрелочки для `type='number'`                                                                                           |
...
| `iconSize?`                                           | `'s'` <code>&#124;</code> `'m'` <code>&#124;</code> `'xs'` <code>&#124;</code> `'l'`                                                                                                                                                                                                                                                                                                                            | высчитывается в зависимости от `size` | Размер иконок                                                                                                                                |
