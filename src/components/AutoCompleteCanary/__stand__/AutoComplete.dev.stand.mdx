...
- [Подсказки при наборе](#подсказки-при-наборе)
  - [Объект с элементами списка](#объект-с-элементами-списка)
...
### Объект с элементами списка

Чтобы описать элементы выпадающего списка, настройте объект `items` типа [DefaultItem](#свойства), где:
...
Чтобы разбить элементы списка на группы, создайте объект с группами и передайте в свойстве `groups` типа [DefaultGroup](#свойства). Что внутри:
...
По умолчанию компонент ищет по названию элемента списка. Чтобы изменить параметры поиска, настройте параметры в `searchFunction`.
...
Чтобы создать кастомное поле ввода или обернуть его в контейнер, используйте вид `clear`. В этом случае поля с разными статусами и разной формы будут выглядеть одинаково.
...
Пример ниже показывает, как можно загрузить данные после раскрытия списка. Этот функционаол стоит использовать, когда список сильно большой.
...
- `onDropdownOpen` - колбек, который отрабатывает каждый раз при открытии/закрытии,
...
| `onScrollToBottom?`                        | `() => void;`                                                                                                                                                                                                                                                                                                                                                                                                   | -                                                        | Колбэк сработает при скролле до конца списка                                                                                                  |
