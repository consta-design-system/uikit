...
const Variants = () => {
  ...
  const linePosition =  useSelect('linePosition', tabsLinePositions, tabsDefaultLinePosition);
  const withIcon =  useBoolean('withIcon', false);
  const onlyIcon =  useBoolean('onlyIcon', false);
  const fitMode = useSelect('fitMode', tabsFitModes, tabsDefaultFitMode, Boolean(linePosition === 'bottom' || linePosition === 'top'));
