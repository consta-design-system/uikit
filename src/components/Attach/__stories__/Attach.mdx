import { 
  AttachExampleBasicsLoading,
  AttachExampleBasicsError,
  AttachExampleBasicsLoaded,
  AttachExampleBasicsLink
  } from './examples/AttachExampleBasics/AttachExampleBasics';
...
import { AttachExampleDescription } from './examples/AttachExampleDescription/AttachExampleDescription';
...
import { AttachExampleAs } from './examples/AttachExampleAs/AttachExampleAs';
...
Есть 4 основных вида компонента: 

Файл загружается. В этот момент показывается лоадер загрузки и опциональное указание процента загрузки. В таком виде у компонента должна быть кнопка отмены с иконкой крестика.

<AttachExampleBasicsLoading />

Файл не загрузился по причине какой-то ошибки. Тут всё просто: красным цветом пишется ошибка и что нужно сделать. В этом виде тоже есть кнопка отмены.

<AttachExampleBasicsError />

Файл успешно загрузился. В этом виде уже точно известно разширение файла и показывается соответсвующая иконка, если есть. Дополнительно можно вывести мета-информацию о файле: размер, дату, автора или что-то ещё. Кнопка отмены подменяется на кнопку удаления с иконкой Корзины. 

<AttachExampleBasicsLoaded />

Просмотр файла. На первый взгляд внешне этот вид не отлчиается от предыдущего. Разница в том, что тут нет никаких кнопок, так как с самим файлом уже нельзя ничего сделать, кроме как открыть (скачать). В этом случае он выполняет роль ссылки или кнопки. 
...
Иконка файла зависит от его расширения, за это отвечает свойство `fileExtension`. Если вы укажете расширение, которого нет в нашей галерее иконок, пользователь увидит иконку с вопросом. [Посмотреть иконки для файлов](?path=/story/components-file--gallery)
...
Свойство `as` отвечает за тег, в который `Attach` оборачивается в HTML-документе.
