...
Чтобы добавить иконки, используйте функцию `getIcon`. В неё нужно передать название элемента массива c иконкой.
...
| [`getLabe?l`](#текст-для-таба)                 | `(item: ITEM) => string, number`           | -            | Функция для формирования текста таба. Используется в качестве уникального ключа элемента                                                                                             |
