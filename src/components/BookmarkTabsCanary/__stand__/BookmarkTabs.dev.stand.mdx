...
Вы можете указать таб как зафиксированный, так он будет рендереиться только с иконкой и будет стоять в начале списка
...
} from '@consta/uikit/BookmarkTabsCanary';
...
Табы заполняют всю доступную ширину. Если что-то не помещается, появляются кнопки прокрутки либо скролл. Это можно настроить с помощью `fitMode`.

### Скролл

Чтобы появилась прокрутка, используйте `fitMode="scroll"` (работает по умолчанию)..

<BookmarkTabsExampleFitModeScroll />

### Кнопки

Чтобы добавить выпадающий список, используйте `fitMode="buttons"`.
...
Чтобы у всех табов (не зафиксированных) появлился крестик для закрытия определите свойство `onRemove`.
...
- `controlRef` - ссылка на элемент (необходимо прикрепить к элементу для скролла по эдементам);
- `label` - лейбл;
- `fixed` - флаг, зафиксирован ли элемент;
- `leftIcon` - иконка слева;
- `rightIcon` - иконка справа;
- `as` - HTML-тег ренедера;
...
| [`fitMode?`](#адаптивность)                  | `'scroll', 'buttons'`                                   | `'scroll'`   | Тип поведения компонента при большом количестве табов                 |
...
| [`onRemove?`](#удаление-табов)               | `(item: ITEM, params: { e: React.MouseEvent }) => void` | -            | Событие при удалении элемента                                         |
