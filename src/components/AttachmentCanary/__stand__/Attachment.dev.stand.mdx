...
import { AttachmentExampleWithIcon } from './examples/AttachmentExampleWithIcon/AttachmentExampleWithIcon';
...
Для того чтобы скрыть иконку укажите значение свойства `withIcon` в `false`.
...
| [`withIcon`](#скрыть-иконку)              | `boolean`                               | `true`       | Отображается ли иконка файла                                     |
...
const App = () => {
  return <Attachment fileName="Документация" extension="doc" withIcon />;
};
```
