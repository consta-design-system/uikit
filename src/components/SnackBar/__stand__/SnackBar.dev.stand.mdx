...
- `progress` добавляет статус [прогресса](#прогресс).
...
Вы можете отображать статус прогресса, укажите в элементе списка `progress` или `getItemProgress` в компоненте. Вид отображения прогресса задается в свойстве `progressView`.
