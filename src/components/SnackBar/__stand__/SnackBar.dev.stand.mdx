...
Добавьте любую из [библиотеки иконок](##LIBS.LIB.STAND/lib:uikit/stand:components-icon-stable) в атрибуте `icon` в свойстве `items` типа [Item](#свойства). Если используете кастомный тип, получить иконку можно с помощью функции `getItemIcon`.
