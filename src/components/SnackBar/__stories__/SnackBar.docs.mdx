...
export const SnackBarExampleStatus: React.FC = () => {
  return <SnackBar items={items} getItemStatus={(item) => getStatus(item.criticality)} />;
};
```
...
export const SnackBarExampleIcon = () => {
  return <SnackBar items={items} getItemIcon={(item) => getIcon(item.status)} />;
};
```
...
export const SnackBarExampleTimer: React.FC = () => {
  ...
  >(reducer, []);
  const generateHandleAdd = (status: SnackBarItemStatus, closeTime?: 'timer' | 'line') => () => {
