...
Свойство `children` задаёт содержимое, которое отображается внутри сайдбара. В нём можно разместить любые React-компоненты или элементы. Для удобства рекомендуем использовать компоненты [`ModalLayout`](##LIBS.LIB.STAND.TAB/lib:uikit/stand:components-modal-stable/tab:dev/hash:modallayout) и [`ModalHeader`](##LIBS.LIB.STAND.TAB/lib:uikit/stand:components-modal-stable/tab:dev/hash:modalheader).
...
Чтобы задать произвольный размер, укажите свойство `size="none"`. Это отключит встроенные стили размера компонента, после чего вы сможете использовать CSS-переменную `--sidebar-size` для кастомизации.
