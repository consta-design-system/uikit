...
function TabsMoreItemsRender(props: TabsMoreItemsProps, ref: React.Ref<HTMLDivElement>) {
              ...
              <div className={cnTabsMoreItems('Content')}>
                {items.map((item) => (
                  <div
                    ...
                    className={cnTabsMoreItems('Item', { active: getItemChecked(item) })}
                  >
                    {renderItem(item, off)}
                  </div>
                ))}
...
export const TabsMoreItems = forwardRef(TabsMoreItemsRender) as TabsMoreItemsComponent;
