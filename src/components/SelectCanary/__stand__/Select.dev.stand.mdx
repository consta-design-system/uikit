...
- [Управление состоянием выпадающего списка](#управление-состоянием-выпадающего-списка)
  - [Пример с раскрытием списка по кнопке](#пример-с-раскрытием-списка-по-кнопке)
  - [Пример с раскрытием списка по фокусу](#пример-с-раскрытием-списка-по-фокусу)
...
Такое API предоставляет не только фильтрацию списка, а абсолютно любые операции.
...
Если необходима только фильтрация списка, то можете воспользоваться хуком `useSearch`. По умолчанию фильтрация происходит по свойству `label` вхождением строки.
...
| [`dropdownForm?`](#форма)                                               | `'default'` <code>&#124;</code> `'brick'` <code>&#124;</code> `'round'` | `default`                 | Форма компонента                                                          |
...
| `dropdownRef?`                                                          | `React.Ref<HTMLDivElement>`                                             | -                         | Ссылка на корневой элемент выпадающего списка                             |
...
| `input?`                                                                | `boolean`                                                               | -                         | Включение поля ввода                                                      |
| `inputValue?`                                                           | `string`                                                                | -                         | Значение поля ввода                                                       |
| `inputDefaultValue?`                                                    | `string`                                                                | -                         | Начальное значение поля ввода                                             |
| `onInput?`                                                              | `(value: string) => void;`                                              | -                         | Событие изменения значения поля ввода                                     |
...
| [`clearButton?`](#кнопка-очистки-поля)                                  | `boolean`                                                               | -                         | Включение кнопки очистки                                                  |
...
| `aria-label?`                                                           | `string`                                                                | -                         | `aria-label` атрибут для поля ввода                                       |
