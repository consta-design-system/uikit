...
За размер тултипа отвечает свойство `size`. Варианты: `s`, `m`, `l`. В зависимости от выбранного размера изменится максимальная ширина тултипа.

## Статус

Иногда может быть полезно дополнительно выделить статус сообщения в тултипе, например для сообщения об ошибке. Сделать это можно с помощью свойства `status` со значениями: `normal`, `success`, `warning` и `alert`.
