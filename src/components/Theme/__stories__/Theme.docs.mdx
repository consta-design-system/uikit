...
У темы есть несколько модификаторов: [шрифт](#шрифты),
[размер текста](#размеры-текста), [отступы](#отступы), [цвета](#цвета), [контролы](#контролы), [тени](#тени).

Каждому модификатору может соответствовать несколько CSS-файлов. Указать, какой из файлов нужно использовать, можно в [пресете](#пресеты).
...
> Чтобы смена тем работала, все значения в CSS нужно задавать переменными, а переменные использовать по смыслу. [Почитать про смысл цветов](https://consta-uikit.vercel.app/?path=/story/components-theme--playground)
...
```json
{
  ...
  "shadow": "myDefault" // указывается значение модификатора _shadow
}
```
