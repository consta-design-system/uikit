...
Основной шрифт в дизайн-системе — Inter, шрифт с открытой лицензией.
[Как его подключить](?path=/docs/common-fonts--page)

Если он вам не подходит, советуем использовать нативные шрифты.

```css
--font-primary: 'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, 'Roboto', 'Oxygen', 'Ubuntu',
...
Модификатор `_shadow` отвечает за тени. Варианты:

- `--shadow-group` — тень для выделения группы объектов на странице, используется для [карточек](?path=/docs/components-card--playground).

- `--shadow-layer` — тень для выделения объектов в отдельный слой, используется для плавающих кнопок, [боковых панелей](?path=/docs/components-sidebar--playground), [выпадающих списков](?path=/docs/components-select--playground) и временно появляющихся элементов интерфейса.

- `--shadow-modal` — тень для выделения отдельных режимов, используется для [модальных окон](?path=/docs/components-modal--playground) и [поповеров](?path=/docs/components-popover--playground).

Посмотреть, как это выглядит, можно в библиотеке [Consta Default Colors](https://www.figma.com/community/file/855572288089401017), во вкладке **Default Shadows**.
