...
```tsx
import { Slider } from '@consta/SliderCanary';
```
...
С помощью `rightSide` можно добавить справа от слайдера любую [иконку](?path=/story/components-icons--playground).
...
| [`leftSide?`](#иконка-и-поле-ввода)                    | `'input'` <code>&#124;</code> `React.FC<IconProps>` | -            | Элемент слева от компонента (иконка)             |
| [`rightSide?`](#иконка-и-поле-ввода)                   | `React.FC<IconProps>`                               | -            | Элемент справа от компонента (поле ввода)        |
