...
Надпись рядом с радиокнопкой можно получить с помощью функции `getLabel` или указав свойство `label` в элементе списка. Лучше писать название с заглавной буквы, точка не нужна.

### Неактивная радиокнопка

Одну или несколько радиокнопок можно сделать неактивными — если вы не хотите, чтобы их выбирали. За неактивность отвечает функция `getDisabled` или указав свойство `disabled` в элементе списка.
