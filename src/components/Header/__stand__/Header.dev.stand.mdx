...
можете добавить свои (но сначала прочитайте, [как это правильно сделать и оформить](##LIBS.LIB.STAND/lib:uikit/stand:custom-contribute).
...
В основе модуля — компонент [Button](##LIBS.LIB.STAND/lib:uikit/stand:components--button--stable) в модификациях: `view_clear`, `size_m`, `only--icon`, `form_round` с [иконкой](##LIBS.LIB.STAND/lib:uikit/stand:components-icon-stable) размера `m`.
