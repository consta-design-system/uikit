...
можете добавить свои (но сначала прочитайте, [как это правильно сделать и оформить](##LIBS.STAND/stand:uikit-custom-contribute-stable).
...
В основе модуля — компонент [Button](##LIBS.STAND/stand:uikit-components-button-stable) в модификациях: `view_clear`, `size_m`, `only-icon`, `form_round` с [иконкой](##LIBS.STAND/stand:uikit-components-icon-stable) размера `m`.
