...
export const TextFieldExampleTypePassword = () => {
  ...
  return <TextField onChange={handleChange} value={value} type="password" placeholder="Пароль" />;
};
```
...
export const TextFieldExampleIncrementButtons = () => {
  ...
  return <TextField onChange={handleChange} value={value} type="number" incrementButtons={false} />;
};
```
