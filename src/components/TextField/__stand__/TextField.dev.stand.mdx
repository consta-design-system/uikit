...
Вы можете сделать поле незаметным: укажите `view = "clear"`. Данный вид используется в случаях когда нужно создать кастомное поле ввода или обернуть его в контейнер, в этом случае
