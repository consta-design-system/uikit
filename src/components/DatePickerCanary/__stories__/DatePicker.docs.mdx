...
- `date-time` — позволяет выбрать одну дату c временем (показывает одно поле ввода),
...
`format` по у молчинию зависит от `type` :

- `type=date`, `date-range` - `'dd.MM.yyyy'`
- `type=date-time` - `'dd.MM.yyyy HH:mm:ss'`

`separator` — точка `'.'`

Разделитель даты и времени не изменен и всегда - пробел `' '`

Разделитель времени не изменен и всегда - двоеточие `':'`
...
**Важно:** если маска содержит дату и время то время всегда должно быть после даты
...
## Множественность часов, минут и секунд
...
| `multiplicitySeconds?`                                                     | `number`                                                                                                                                                            | 1                     | Интервал для вывода секунд (1 — показать все секунды, 0 — убрать секунды) см. [DateTime](/?path=/docs/components-datetime--playground) |
| `multiplicityMinutes?`                                                     | `number`                                                                                                                                                            | 1                     | Интервал для вывода минут (1 — показать все минуты, 0 — убрать минуты) см. [DateTime](/?path=/docs/components-datetime--playground)    |
| `multiplicityHours?`                                                       | `number`                                                                                                                                                            | 1                     | Интервал для вывода часов (1 — показать все часы, 0 — убрать часы) см. [DateTime](/?path=/docs/components-datetime--playground)        |
