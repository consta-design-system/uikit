...
- `date-time-range` — позволяет выбрать период cо временем (показывает два поля ввода),
- `time` — позволяет выбрать время (показывает одно поле ввода).
...
| `onFocus?`                                                                 | `React.FocusEventHandler<HTMLElement>` &#124; `[React.FocusEventHandler<HTMLElement>?, React.FocusEventHandler<HTMLElement>?]`                                                                                                                              | -                     | Событие по фокусу в поле, см. [TextField](?path=/docs/components-textfield--playground)                                |
| `onBlur?`                                                                  | `React.FocusEventHandler<HTMLElement>` &#124; `[React.FocusEventHandler<HTMLElement>?, React.FocusEventHandler<HTMLElement>?]`                                                                                                                              | -                     | Событие при потере фокуса в поле, см. [TextField](?path=/docs/components-textfield--playground)                        |
...
| `inputRef?`                                                                | `React.Ref<HTMLInputElement>` &#124; `[React.Ref<HTMLInputElement>?, React.Ref<HTMLInputElement>?]`                                                                                                                                                         | -                     | Ссылка на DOM-элемент поля ввода, недоступна при выборе диапазона дат                                                  |
