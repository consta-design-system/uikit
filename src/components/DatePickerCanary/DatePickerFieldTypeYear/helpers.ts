...
type DatePickerFieldTypeYearPropOnChange = (props: { e: Event; value: Date | null }) => void;
    ...
    if (inputRef.current) {
      imaskRef.current = (IMask(inputRef.current, {
        ...
        validate: (string: string) => {
          const [yyyy] = getPartsDate(string, formatProp, separator, false, ['yyyy']);

          if (yyyy && !isValid(parse(`${yyyy}`, datePickerPropFormatTypeYear, new Date()))) {
            onError &&
              onError({
