...
Надпись рядом с чекбоксом можно получить с помощью функции `getLabel` или указав в объектах списка поле `label`. Лучше писать название с заглавной буквы, точка не нужна.
...
Один или несколько чекбоксов можно сделать неактивными — если вы не хотите, чтобы их выбирали. За неактивность отвечает функция `getDisabled` или поле `disabled` в элементе списка.
