...
Текст для всех кнопок указывается в `items` и формируется с помощью функции `getLabel` или в поле `label` элемента списка. Например, так:
...
Чтобы показать иконку на кнопке, используйте `getIcon` или поле `icon` элемента списка.
...
За неактивность отвечает функция `getDisabled` или поле `disabled` элемента списка.
