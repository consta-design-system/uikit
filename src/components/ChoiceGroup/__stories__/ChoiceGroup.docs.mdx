...
import { ChoiceGroupExampleAdaptive } from './examples/ChoiceGroupExampleAdaptive/ChoiceGroupExampleAdaptive';
...
Если группа кнопок слишком большая или текст на кнопке слишком длинный и не помещается в выделенную область, текст на кнопке сокращается, в конце появляется многоточие.
...
Если компонент не помещается в родительский контейнер по ширине, он сжимается,
текст обрезается, в конце добавляется многоточие. [Посмотреть пример](#слишком-длинный-текст)
