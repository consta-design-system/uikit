...
В нашей дизайн-системе несколько скелетонов, с их помощью можно сделать плейсолдеры для разных компонентов. Например, на месте этого скелетона появятся аватарка и текст — когда загрузятся.

<SkeletonExampleAvatar />

- [Прямогуольный скелетон](#прямоугольный-скелетон)
