...
Если используете компоненты для лэйаута (например, [Grid](##LIBS.STAND/stand:uikit-components-grid-stable)), положите внутрь скелетоны, а после загрузки поменяйте их на реальные компоненты.
