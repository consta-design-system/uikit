...
В случаях когда вы не хотите отоброжать все страницы сразу вы можете рендерить только необходимое вам количество, задаваемое через `visibleCount`.
...
Если у вас большое количество страниц и вы хотите дать пользователю видеть количство страниц и дать перходить на первую и последняю укажите свойства `showFirstPage` и `showLastPage` соответственно.
...
Для того чтобы отобразить кнопки перехода в конец или начало определите свойство `outerMostArrows`. Оно принимает массив двух значений, где может быть:
...
Для того чтобы отобразить кнопки для перехода по страницам вперед назад определите свойство `arrows`. Оно принимает массив двух значений, где может быть:
...
| `getItemOnClick?`                       | `PaginationPropGetItemOnClick`                                           | -            | Функция для получения события нажатя на элемент                       |
