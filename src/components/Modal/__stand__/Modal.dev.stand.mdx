...
import { ModalLayoutExample } from './examples/ModalLayoutExample/ModalLayoutExample';
import { ModalExampleZIndex } from './examples/ModalExampleZIndex/ModalExampleZIndex';
import { ModalHeaderExample } from './examples/ModalHeaderExample/ModalHeaderExample';
import { ModalExampleFullScreen } from './examples/ModalExampleFullScreen/ModalExampleFullScreen';
...
- [ModalLayout](#modallayout)
  - [Импорт](#импорт-modallayout)
  - [Свойства](#свойства-modallayout)
  - [Пример использования](#пример-использования-modallayout)
- [ModalHeader](#modalheader)
  - [Импорт](#импорт-modalheader)
  - [Свойства](#свойства-modalheader)
  - [Пример использования](#пример-использования-modalheader)
...
| `container?`                         | `HTMLElement \| React.RefObject<HTMLElement>` | `window.document.body` | Родительский контейнер для модального окна                  |
...
Модальное окно не имеет фиксированных размеров но и не может быть больше контейнера в котором он рендерится. По умолчанию модальное окно имеет отступы от родительного контейнера `5vh` по вертикали и `5vw` по горизонтали. Отступы можно изменить с помощью css переменной `--modal-window-offset-prop`.

Пример с раскрытием модального окна во весь экран:
...
Компонент для разметки содержимого модального окна.

## Импорт ModalLayout

```tsx
import { ModalLayout } from '@consta/uikit/ModalLayout';
```

## Свойства ModalLayout

| Свойство     | Тип или варианты значения                         | По умолчанию | Описание                                                                                |
| ------------ | ------------------------------------------------- | ------------ | --------------------------------------------------------------------------------------- |
| `children?`  | `React.ReactNode`                                 | -            | Содержимое, отображаемое модального окна, крайние ноды воспринимаются как шапка и футер |
| `fixed`      | `boolean \| [boolean, boolean]`                   | -            | Фиксирует шапку и(или) футер в верхней или нижней части модального окна соответственно  |
| `border?`    | `boolean \| [boolean, boolean]`                   | -            | Отображает границы между секций модального окна                                         |
| `space?`     | `MixSpaceProps \| (MixSpaceProps \| undefined)[]` | -            | Отступы для всех секций модального окна                                                 |
| `className?` | `string`                                          | -            | Дополнительный CSS-класс для модального окна                                            |

## Пример использования ModalLayout
...
```tsx
<Modal isOpen hasOverlay={false} style={{ height: 250, width: 400 }} border>
...
Компонент для разметки шапки модального окна.

## Импорт ModalHeader

```tsx
import { ModalHeader } from '@consta/uikit/ModalHeader';
```

## Свойства ModalHeader

| Свойство     | Тип или варианты значения                                   | По умолчанию   | Описание                                           |
| ------------ | ----------------------------------------------------------- | -------------- | -------------------------------------------------- |
| `children`?  | `ReactNode`                                                 | -              | Текст заголовка модального окна                    |
| `className?` | `string`                                                    | -              | Дополнительный CSS-класс для шапки модального окна |
| `icon?`      | `IconComponent`                                             | -              | Иконка, отображаемая в заголовке                   |
| `onClose?`   | `() => void`                                                | -              | Обработчик события закрытия модального окна        |
| `direction?` | `'vertical' \| 'horizontal'`                                | `'horizontal'` | Направление расположения элементов                 |
| `size?`      | `'s' \| 'm' \| 'l' \| 'xs'`                                 | -              | Размер текста заголовка                            |
| `status?`    | `'success' \| 'system' \| 'normal' \| 'warning' \| 'alert'` | -              | Статус иконки                                      |
| `ref?`       | `Ref<HTMLDivElement>`                                       | -              | Ref для доступа к DOM элементу                     |

## Пример использования ModalHeader
...
```tsx
<Modal style={{ width: 400 }} hasOverlay={false} isOpen border>
  <ModalLayout space={{ p: 'm' }} border={[true, false]}>
    <ModalHeader onClose={() => {}} icon={IconAllDone} status="success">
      Это заголовок модального окна
    </ModalHeader>
    Это содержимое модального окна. Здесь может быть что угодно: текст, изображение,
    форма или таблица. Всё, что хочется вынести из контекста и показать поверх основной
    страницы.
  </ModalLayout>
</Modal>
```
