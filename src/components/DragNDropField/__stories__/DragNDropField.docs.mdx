...
| `fit-files`      | `string`                      | `Подходят файлы`                               | Показывается, если есть ограничения                              |
| `before`         | `string`                      | `до`                                           | Показывается, если есть ограничения по размеру и формату         |
| `max`            | `string`                      | `Максимум`                                     | Показывается, если есть ограничения по только по размеру         |

#### Тексты ошибок

| Свойство            | Тип и значения | По умолчанию                                                   | Описание                                                      |
| ------------------- | -------------- | -------------------------------------------------------------- | ------------------------------------------------------------- |
| `file-invalid-type` | `string`       | `формат файла не подходит`                                     | Ошибка при неподходящем формате файла                         |
| `file-too-large`    | `string`       | `файл слишком большой (максимум {размер + единица измерения})` | Ошибка, если файл больше `max`                                |
...
| `general-error`     | `string`       | `не получилось добавить файл`                                  | Общая ошибка, если файл не загрузился по какой-то причине     |
