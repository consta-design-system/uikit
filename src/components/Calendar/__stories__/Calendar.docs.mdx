...
> Свойство `value` описан в [CalendarPropValue](#свойства)

> Событие выбора даты описан в [CalendarPropOnChange](#свойства)

### Пример выбора периода дат
...
export const CalendarExamplePeriod = () => {
  ...
  return <Calendar value={rangeValue} onChange={handleChange} />;
};
```
...
export type CalendarPropValue = Date | [Date?, Date?];
