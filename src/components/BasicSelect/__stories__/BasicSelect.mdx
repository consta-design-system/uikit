...
import { BasicSelectExampleSize, BasicSelectExampleSizeWidth } from './examples/BasicSelectExampleSize/BasicSelectExampleSize';
...
Для работы компоненту нужно передать тип опции (`Option`): `<BasicSelect<T> {..props}/>`.
