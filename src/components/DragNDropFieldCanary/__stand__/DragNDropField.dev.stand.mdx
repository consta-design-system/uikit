...
В компонент можно передать четыре коллбэка для обработки загруженных файлов и ошибок:
...
> Коллбэк будет вызван независимо от того, были файлы приняты или отвергнуты.
...
Коллбэк вызывается, если хотя бы один файл соответствует всем заданным условиям. Если ни один файл не прошел по условиям, вызван не будет. Все файлы, удовлетворяющие условиям, будут собраны в массив объектов типа [`File`](https://developer.mozilla.org/en-US/docs/Web/API/File).
...
Коллбэк вызывается, если хотя бы один файл был отвергнут в соответствии с заданными условиями. Если файлы, которые загружает пользователь, не подходят под ограничения, компонент вернет массив отказов типа `FileRejection`.
...
| `loading?`       | `boolean` &#124; `number`           | Лоадер загрузки, числовое значение покажет степень загрузки, значение `true` заставит спиннер крутиться бесконечно. Значение по умолчанию: `false`. |
