...
`DragNDropField` — область для загрузки файлов путем перетаскивания или выбора из диалогового окна.

## Содержимое

Компонент является контейнером: всё, что вы добавите в него, пользователь увидит внутри рамки для загрузки файлов.
...
Если оставить компонент пустым, в рамке будет содержимое по умолчанию (текст и кнопка):
...
1. onDrop - обработчик события дропа. Файлы принимаются или отвергаются, основываясь на свойствах `accept`, `multiple`, `minSize` и `maxSize`.
   Заметьте, что колбек будет вызван независимо от того, были файлы приняты или отвергнуты.
   Если необходимо реагировать на специфический сценарий, используйте `onDropAccepted` или `onDropRejected`.
...
2. onDropAccepted - вызывается, если хотя бы один файл соответствует всем заданным условиям. Если ни один файл не прошел по условиям, вызван не будет.

```tsx
onDropAccepted?: <T extends File>(files: T[]) => void;
```

3. onDropRejected - вызывается, если хотя бы один файл был отвергнут в соответствии с заданными условиями. Если ни один файл не был отвергнут, вызван не будет.
...
4. onError - обработчик других ошибок промиссов.
...
В этом примере реализован механизм накапливания загруженных файлов. Обратите внимания, что компонент не накапливает файлы сам, только возвращает результаты одного дропа для дальнейшей обработки.
...
| [`onDropAccepted?`](#обработка-загрузки-файлов) | `(files: File[]) => void`                                                             | -            | Обработчик события загрузки файлов (если ни один файл не прошел по условиям, вызван не будет) |
| [`onDropRejected?`](#обработка-ошибок)          | `(fileRejections: FileRejection[], event: DropEvent) => void`                         | -            | Обработчик события отказа файлов (если ни один файл не был отвергнут, вызван не будет)        |
