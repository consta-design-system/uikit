...
| [`locale?`](#локализация)                        | `DateTimePropLocale`                                   | `ru`         | Локализация календаря                                                      |
...
Свойство `locale` позволяет настроить язык календаря с использованием библиотеки [date-fns](https://date-fns.org/). По умолчанию используется русская локализация (`ru`).
