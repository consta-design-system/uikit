...
> Сначала прочитайте всё [о темах](##LIBS.STAND/stand:uikit-theme-themeabout-stable)
...
[Подробнее про модификаторы](##LIBS.STAND.TAB/tab:dev/stand:uikit-components-theme-stable/hash:модификаторы)
...
Это нужно, так как некоторые компоненты используют инверсные цвета (например, [Tooltip](##LIBS.STAND/stand:uikit-components-tooltip-stable)).
...
Содержит сетку размеров шрифта и межстрочных интервалов. Значения используются в компоненте [`Text`](##LIBS.STAND.TAB/tab:dev/stand:uikit-components-text-stable) и всех текстовых
