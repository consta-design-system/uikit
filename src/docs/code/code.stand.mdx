...
1. компонент должен [лежать в папке с дочерними компонентами](##LIBS.LIB.STAND/lib:uikit/stand:workflow-filestructure) (не быть в том же файле),
...
Если в компоненте используется иконка, которой нет в UI-ките, её можно создать через `@consta/uikit/createIcon`. [Подробнее о создании иконок](##LIBS.LIB.STAND/lib:uikit/stand:custom-addicons)
