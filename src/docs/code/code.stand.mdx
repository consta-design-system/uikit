...
1. компонент должен [лежать в папке с дочерними компонентами](##LIBS.STAND|stand:uikit-workflow-filestructure-stable) (не быть в том же файле),
...
Если в компоненте используется иконка, которой нет в UI-ките, её можно создать через `@consta/uikit/createIcon`. [Подробнее о создании иконок](##LIBS.STAND/stand:uikit-custom-addicons-indefined)
