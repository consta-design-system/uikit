...
```tsx
import { useResizableContent } from '@consta/uikit/useResizableContentCanary';
```

С помощью хука `useResizableContent` размеры блоков в контейнере можно сделать изменяемыми.

> Для избежания пересоздания внутринних функций хука, мемоизируйте входные параметры.
