```tsx
import { useBreakpoints } from '@consta/uikit/useBreakpointsCanary';
```
...
```tsx
const breakpoints = useBreakpoints({ map: { isDesktop: 1000 } });
...
import {
  useBreakpoints,
  getLastPoint,
} from '@consta/uikit/useBreakpointsCanary';
...
const lastPoint = getLastPoint(useBreakpoints({ map: { m: 300, l: 500 } }));
...
const breakpoint = getLastPoint(useBreakpoints({ map: { m: 300, l: 500 } }));
...
| ------------ | --------------------------------------------------------- | ------------ | ---------------------------------------------------- |
| `isActive?`  | `boolean`                                                 | `true`       | Включает хук                                         |
| `container?` | `React.RefObject<HTMLElement>`<code>&#124;</code>`Window` | `window`     | Контейнер, относительно которого вычисляются размеры |
| `map?`       | `Record<POINTS, number>`                                  | -            | Набор контрольных точек                              |

### createMods

Хелпер `addPrefix` добавляет префикс для наименований брейкпоинтов для дальнейшего использования их в функции `cn`

```tsx
import { useBreakpoints, addPrefix } from '@consta/uikit/useBreakpointsCanary';

const breakpoints = useBreakpoints({ map: { l: 500, xl: 800, 2xl:1000 }});
