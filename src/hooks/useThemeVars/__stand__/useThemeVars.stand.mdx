...
```tsx
export { useThemeVars } from '@consta/uikit/useThemeVars';
```
...
чтобы пересчёт происходил, когда вам нужно. Например, если вы используете в теме медиазапросы,
и размер шрифта зависит от ширины экрана, тогда вам нужно прописать в `useThemeVars` текущую граничную точку (breakpoint) как зависимость.
