...
Пересчет переменных зависит от [модификаторов темы](##LIBS.STAND.DEV/stand:uikit-components-theme-stable/hash:модификаторы) (цвета, размеры элементов, шрифты, отступы и так далее). Если эти модификаторы не меняются, пересчёт не происходит.
