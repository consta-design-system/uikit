...
<Meta title="Mixins|MixPopoverAnimate" id="MixPopoverAnimate" />
...
Миксин можно применить к компонентам из библиоеки [react-transition-group](http://reactcommunity.org/react-transition-group/)
...
| `direction?` | `Direction`                                                                                                                      | -            | Позиция окна |
...
export const MixPopoverAnimateExample = () => {
  ...
  return (
    <>
      <Button label="Нажми" onClick={setOpen.toogle} ref={buttonRef} />
      <Transition in={open} unmountOnExit timeout={animateTimeout} nodeRef={popoverRef}>
        {(animate) => {
          return (
            <Popover
              className={cnMixPopoverAnimate({ direction, animate })}
              offset={8}
              anchorRef={buttonRef}
              ref={popoverRef}
  ...
  return (
    <>
      ...
      <CSSTransition
        ...
        nodeRef={popoverRef}
      >
        <Popover offset={8} anchorRef={buttonRef} ref={popoverRef} direction="downStartLeft">
          я появился плавно <br /> c анимацией 🔥
        </Popover>
      </CSSTransition>
    </>
