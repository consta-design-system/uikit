...
const CopyButton = (props: { copied?: boolean, onClick: React.MouseEventHandler }) => {
    ...
    return (
        <div className={cnCode('Copy')}>
            <button type="button" onClick={onClick} className={cnCode('CopyButton', [cnIcon({ size: 'xs' }), cnMixFocus()])}>
                <CSSTransition
                    in={!copied}
                    unmountOnExit
    ...
    if (className) {
        return (
            <div className={cnCode(null, [className])}>
                <Highlight {...otherProps} className={className}>{children}</Highlight>
