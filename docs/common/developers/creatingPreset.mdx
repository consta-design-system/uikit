...
в вашем проекте необходимо создать директорию для нового(ых) пресетов.
Например это может быть ```src/themes/```. 
...
Создание нового пресета подразумевает ручную настройку css переменных (также их принято называть *дизайн токенами*), составляющих темы.  
...
Здесь подключаются все начертания/насыщенности/weight шрифта. Убедитесь, что в шрифте есть насыщенность, которую вы используете. 
...
import { myNewPreset } from './themes/myPresetDefault'
